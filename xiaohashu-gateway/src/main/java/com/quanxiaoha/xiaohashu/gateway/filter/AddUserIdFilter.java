...
import com.quanxiaoha.xiaohashu.gateway.constant.RedisConstants;
...
public class AddUserIdFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        ServerWebExchange newExchange = exchange.mutate()
                .request(builder -> builder.header(GlobalConstants.UserId, String.valueOf(userId))) // 将用户 ID 设置到请求头中
                .build();
        return chain.filter(newExchange);
    }

    private Long getUserId(ServerWebExchange exchange) throws Exception {
        log.info("=======begin to get userId from self-defined method {}");
        //get token
        List<String> tokenList = exchange.getRequest().getHeaders().get(TOKEN_HEADER_KEY);
        ...
        String redisKey = RedisConstants.SA_TOKEN_TOKEN_KEY_PREFIX + token;
        Long userId = (Long)redisTemplate.opsForValue().get(redisKey);
        log.info("=======get userId from self-defined method {}",userId);
        return userId;
    }
}
